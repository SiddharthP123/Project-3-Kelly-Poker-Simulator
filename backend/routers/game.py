...
from backend.models import BankrollLog, GameSession, HandHistory
...
def _get_session_or_404(session_id: int, db: Session) -> GameSession:
...
def create_game_session(request: Request, body: CreateGameSessionRequest, db: Session = Depends(get_db)):
    session = GameSession(
...
def get_game_session(request: Request, session_id: int, db: Session = Depends(get_db)):
...
@limiter.limit(READS_LIMIT)
def get_bankroll_history(request: Request, session_id: int, db: Session = Depends(get_db)):
    _get_session_or_404(session_id, db)
    return (
...
@limiter.limit(WRITES_LIMIT)
def end_game_session(request: Request, session_id: int, db: Session = Depends(get_db)):
    session = _get_session_or_404(session_id, db)
