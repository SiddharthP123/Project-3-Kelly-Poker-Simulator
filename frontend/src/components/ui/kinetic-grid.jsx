...
/**
 * Ambient full-screen canvas background -- a dot grid that warps toward
 * the cursor and ripples on click. Adapted from the 21st.dev "Kinetic
 * Grid" bookmark; children render above it (z-10) so it reads as an
 * animated backdrop, not an interactive layer of its own (the canvas is
 * pointer-events-none -- clicks/moves are read from `window`, so nothing
 * underneath ever has its own interactivity blocked).
 */
const KineticGrid = ({ children, className, globalColor = 'default' }) => {
